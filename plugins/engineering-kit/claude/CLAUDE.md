...
Org-wide rules for AI-assisted development, reaching a project by one of three
routes: imported into its `CLAUDE.md` via
`@vendor/scrapkit/engineering-kit/claude/CLAUDE.md`, condensed into the agent
files by Laravel Boost (from `resources/boost/guidelines/core.blade.php`), or
injected at session start by the Claude Code plugin where Composer does not
reach. The routes never conflict — this file is the authoritative source and
the plugin stays quiet wherever the Composer import is already in place.
Project-specific rules live in the project's own `CLAUDE.md` and take
precedence over this file.

Where the package is installed, the full standards are in
`vendor/scrapkit/engineering-kit/docs/` — consult `coding-guidelines.md` and
`architecture-guidelines.md` when a decision is not covered here, and
`package-guidelines.md` before versioning or releasing a `scrapkit/*` package.
Where it is not, the `engineering-kit` skills carry the guidelines they need in
their own `references/` directory; there is no other copy to look for.
